...
public class WebMvcConfig implements WebMvcConfigurer{
	...
	public void addCorsMappings(CorsRegistry registry) {
		...
		registry.addMapping("/**")
			//allowedOrigins("http://localhost:3000") : React어플리케이션이 실행되는 도메인에서 오는 요청을 허용
			.allowedOrigins("http://localhost:3000")
			//.allowedMethods("GET", "PUT", "POST", "DELETE") : HTTP 메서드를 허용
			.allowedMethods("GET", "PUT", "POST", "DELETE")
			//.allowedHeaders("*") : 모든 헤더를 허용
			.allowedHeaders("*")
			//.allowCredentials(true) : 쿠키가 인증 정보를 포함한 요청을 허용
			.allowCredentials(true)
			//.maxAge(MAX_AGE_SECS) : 브라우저가 서버로부터 받은 응답을 일정 시간 동안 그 시간내에 동일한 요청이 있을 경우
			//						  서버에 다시 요청하지 않고 저장된 응답을 재사용 함
			.maxAge(MAX_AGE_SECS);
	}
}
